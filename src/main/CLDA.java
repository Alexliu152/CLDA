...
public class CLDA implements CLDAModel {
    ...
    public CLDA() {
        ...
        config = Configuration.getConfig().properties;
        ...
        try {
            ...
            logger.warn("Create file failed");
        }
        // </editor-fold>
  ...
  /* Build the corpus for later use from
   *        json files (user defined)
   *  to
   *        line-segmented document
   */
    ...
    private static String SegmentComment(String comment, int limit) {
        String result = "";
        try {
            List<Word> parsedAnswer = WordSegmenter.seg(comment);

            if (parsedAnswer.size() <= limit)
                return result;
    ...
    public static void main(String[] args) {
        ...
        switch (testCase) {
            ...
            case COM:
                ...
                test.ConvertLine2Vector(comment, test.config.getProperty("Comment"), counterModel);
                break;
            case BID:
            ...
            case PRE:
                counterModel = CountVectorizerModel.load(test.config.getProperty("CVModel"));
                test.Predict(comment, test.config.getProperty("LDAModel"), counterModel, 5);
                break;
            default:
                break;
        }
  ...
  /* Transfer each segmented comment to its vector representation */
    ...
    @Override
    public void BuildCorpus(String input, String output_file) {

        File[] inputFiles = ListDir(input);
        JsonParser parser = new JsonParser();

        FileOutputStream fileOutputStream = null;

        // <editor-fold desc="Open the output file">
        try {
            fileOutputStream = new FileOutputStream(output_file);
        } catch (FileNotFoundException e) {
            logger.warn("Corpus output file error");
            return;
        }
        //</editor-fold>

        // <editor-fold desc="Convert & Write">
        try {
               ...
               */
                            ...
                            for (int index = 1; index <= 99; index++) {
                                ...
                                if (answerLine != null && answerLine.length() >= 50) {
                                    String segmentedComment = SegmentComment(answerLine, 100);
    ...
    public void ConvertLine2Vector(String line, String output,
                                   CountVectorizerModel vectorizerModel) {

        List<Word> parsedAnswer = WordSegmenter.segWithStopWords(line);
    ...
    /*
     *
     * Convert list to data-set in spark
     *
     */
        ...
        try {
            ...
            logger.warn("Segmentation file not found");
        } catch (IOException e) {
            logger.warn("Read segmentation result error");
        }
    ...
    /* Vectorize the training data */
        ...
        try {
            ...
            logger.warn("Counter model save error");
        }
    ...
    /* Output the data */
        ...
        for (Row r : vectorizedData.select("vector").collectAsList()) {
            ...
            // <editor-fold desc="Dense Vector String">
            StringBuilder stringBuilder1 = new StringBuilder();
            stringBuilder1.append(String.valueOf(index++)).append(" ");
            int index2 = 1;
            for (String number : featureStringVector) {
                stringBuilder1.append(String.valueOf(index2)).append(":").append(String.valueOf(((Double) Double.parseDouble(number)).intValue())).append(" ");
                index2++;
        ...
        try {
            ...
            logger.warn("File close error");
        }
    }

    /* Build the model */
    @Override
    public void Build(String input, String output, int topicNum) {
        // Loads data.
        Dataset<Row> dataSet = session.read().format("libsvm").load(input);
        dataSet.show();
    ...
    @Override
    public void Predict(String line, String modelURI, CountVectorizerModel counterModel, int number) {
        LocalLDAModel sameModel = LocalLDAModel.load(modelURI);
        Matrix topics = sameModel.topicsMatrix();

        ConvertLine2Vector(line, config.getProperty("Comment"), counterModel);
